...
describe("fixed gateway", () => {
  ...
  it("accepts only the verified HTTPS origin and /v1 path", () => {
    ...
    process.env.GATEWAY_BASE_URL = "https://homepilot-ai.shares.zrok.io:444/v1";
  ...
  it("sends both credentials, disables provider storage, and tracks separate IDs", async () => {
    ...
    const fetchMock = vi.fn(async (url: URL | RequestInfo, init?: RequestInit) => {
      ...
      return new Response(JSON.stringify({ id: "resp_1", status: "completed", model: "gpt-5.4-mini", output_text: "OK", usage: { input_tokens: 2, output_tokens: 1 } }), { status: 200, headers: { "X-Request-Id": "req_1" } });
    });
  ...
  it("requires an exact normalized health response", async () => {
    process.env.CLIPROXY_API_KEY = "cli-test";
    process.env.HOME_GATEWAY_SECRET = "gateway-test";
    vi.stubGlobal("fetch", vi.fn(async () => new Response(JSON.stringify({ status: "completed", output_text: "NOT OK", model: "gpt-5.4-mini" }), { status: 200 })));
