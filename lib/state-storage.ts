...
export interface StateRead { state: ControlCenterState; etag?: string }
...
function storageError(action: "read" | "write", error: unknown): AppError {
  ...
  return new AppError(
    `Persistent storage ${action} failed. Verify the private Blob connection and redeploy the project.`,
    503,
    `storage_${action}_failed`
  );
}
...
export async function readState(): Promise<StateRead> {
  ...
  if (mode === "blob-token" || mode === "blob-oidc") {
    try {
      ...
      if (error instanceof SyntaxError) throw new AppError("Stored control-center state is invalid.", 503, "storage_state_invalid");
      throw storageError("read", error);
    }
  }
  if (mode === "unavailable") return { state: emptyState() };
  try {
    return { state: normalizeState(JSON.parse(await readFile(LOCAL_PATH, "utf8"))) };
  } catch (error) {
...
export async function writeState(state: ControlCenterState, etag?: string): Promise<void> {
  ...
  if (mode === "unavailable") {
    throw new AppError("Connect a private Blob store to this Vercel deployment and redeploy Production.", 503, "storage_not_configured");
  }
