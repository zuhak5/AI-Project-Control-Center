...
interface Result { text: string; model: string; latencyMs: number; inputTokens: number; outputTokens: number; requestId: string | null; statusCode: number; raw: unknown }
...
export function PlaygroundClient({ model, maxOutputTokens }: { model: string; maxOutputTokens: number }) {
  ...
  return <div className="playground-grid">
    <form className="panel form-grid compact" onSubmit={async (event) => {
      ...
      const response = await fetch("/api/playground", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ prompt, system, maxOutputTokens: limit }) });
      ...
      <div className="field"><label htmlFor="limit">Maximum output tokens</label><input id="limit" type="number" min="16" max="8000" value={limit} onChange={(event) => setLimit(Number(event.target.value))} /></div>
    ...
    <section className="panel response-panel">
      ...
      {result ? <><div className="response-text">{result.text}</div><dl className="response-meta"><div><dt>Model</dt><dd>{result.model}</dd></div><div><dt>HTTP</dt><dd>{result.statusCode}</dd></div><div><dt>Latency</dt><dd>{result.latencyMs} ms</dd></div><div><dt>Input tokens</dt><dd>{result.inputTokens}</dd></div><div><dt>Output tokens</dt><dd>{result.outputTokens}</dd></div><div><dt>Request ID</dt><dd>{result.requestId ?? "Not reported"}</dd></div></dl><details><summary>Raw gateway JSON</summary><pre>{JSON.stringify(result.raw, null, 2)}</pre></details></> : <div className="response-placeholder"><span>⌁</span><p>Run a request to verify the complete Google Cloud VM gateway path.</p></div>}
    </section>
  </div>;
}
